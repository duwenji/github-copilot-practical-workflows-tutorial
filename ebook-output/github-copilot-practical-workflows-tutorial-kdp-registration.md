# KDP Registration Package: GitHub + GitHub Copilot 実務シナリオ完全チュートリアル

- Generated: 2026-07-04 23:50:43
- Project: github-copilot-practical-workflows-tutorial
- Metadata: C:\Dev\tutorials\github-copilot-practical-workflows-tutorial\.\.github\skills-config\ebook-build\github-copilot-practical-workflows-tutorial.metadata.yaml
- KDP Metadata: C:\Dev\tutorials\github-copilot-practical-workflows-tutorial\.\.github\skills-config\ebook-build\github-copilot-practical-workflows-tutorial.kdp.yaml

## 1. Bibliographic Information

| Field | Value |
|---|---|
| Title | GitHub + GitHub Copilot 実務シナリオ完全チュートリアル |
| Title Pronunciation | — |
| Romanized Title | — |
| Subtitle | 日常開発、協業、共有資産管理、リリース運用を一気通貫で学ぶ |
| Subtitle Pronunciation | — |
| Romanized Subtitle | — |
| Author | 杜 文吉 |
| Language | ja-JP |
| Publisher | GitHub |
| Rights | Creative Commons Attribution 4.0 International License |
| Identifier | github-copilot-practical-workflows-tutorial-v1 |
| Publication Date | 2026-04-04 |
| ISBN | Typically not required for Kindle only |

## 2. Product Description

> GitHub と GitHub Copilot を、日常開発・Pull Request・fork/upstream・submodule/subtree・release/hotfix といった実務シナリオに沿って学ぶ日本語教材です。 単発機能ではなく、チーム開発に必要な運用の流れを一冊で理解できるよう構成しています。

## 3. Keywords (up to 7)

- GitHub
- GitHub Copilot
- Pull Request
- Git Workflow
- Submodule
- Release Management
- Team Development

## 4. Categories (up to 3)

- Computers / Software Development / Tools
- Computers / Programming / General
- Computers / Project Management

## 5. Pricing and Distribution

| Field | Value |
|---|---|
| List Price | 1980 JPY |
| Royalty | 70% |
| Territories | Worldwide |
| KDP Select | No |

## 6. Upload Files

- EPUB: C:\Dev\tutorials\github-copilot-practical-workflows-tutorial\.\ebook-output\github-copilot-practical-workflows-tutorial.epub
- PDF: C:\Dev\tutorials\github-copilot-practical-workflows-tutorial\.\ebook-output\github-copilot-practical-workflows-tutorial.pdf
